...
from bot.utils.donation_reply import (
    build_random_donation_club_inline_button,
...
def _build_campaign_keyboard(buttons: List[Dict[str, Any]]) -> Optional[InlineKeyboardMarkup]:
    """Собирает inline-клавиатуру кампании; meta-кнопка → случайный донат/клуб."""
    if not buttons:
        return None

    keyboard_buttons: List[List[InlineKeyboardButton]] = []
    add_donation_club = False

    for btn in buttons:
        if is_donation_club_random_meta(btn):
            add_donation_club = True
            continue
        if btn.get("url"):
    ...
    if add_donation_club:
        keyboard_buttons.append([build_random_donation_club_inline_button()])
...
class MailingFeature(BaseFeature):
    ...
    async def _check_ready_campaigns(self):
        ...
        try:
            ...
            if campaigns:
                logger.info(
                    "📬 Mailing: готовы к отправке кампании id=%s",
    ...
    async def _run_campaign(self, campaign_id: int):
        ...
        try:
            ...
            logger.warning(
                "📤 Starting campaign %s: %s users, rate=%s msg/sec, source=%s",
                campaign_id,
                total_users,
                current_rate,
                campaign.get("campaign_source") or "manual",
            )
    ...
    async def _send_to_user(
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                keyboard = _build_campaign_keyboard(campaign.get("buttons", []))
                ...
                buttons_raw = campaign.get("buttons") or []
                has_donation_club_btn = any(
                    is_donation_club_random_meta(b) for b in buttons_raw
                )
                if not has_donation_club_btn:
                    try:
                        await self.user_storage.set_show_donation_flag(user_id, True)
                    except Exception as e:
                        logger.debug("set_show_donation_flag after mailing: %s", e)
                try:
                    await self.mailing_storage.bump_user_mailing_after_send(user_id, sent=True)
                except Exception as e:
                    logger.debug("bump_user_mailing_after_send: %s", e)
                return True, None
