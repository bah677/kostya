...
from bot.payments.subscription_checker import SubscriptionChecker
...
class TelegramBotApp:
    ...
    def __init__(
        ...
        *,
        ...
        self.subscription_checker: Optional[SubscriptionChecker] = None
    ...
    async def _start_background_tasks(self) -> None:
        try:
            if self.payment_feature is not None:
                ...
                if config.DONATION_RECURRING_ENABLED:
                    ...
                    await self.subscription_checker.start()
                else:
                    self.subscription_checker = None
                    logger.info(
                        "SubscriptionChecker не запущен (DONATION_RECURRING_ENABLED=false)"
                    )
            else:
    ...
    async def _stop_background_tasks(self) -> None:
        try:
            ...
            if self.subscription_checker:
                await self.subscription_checker.stop()
                logger.info("✅ SubscriptionChecker остановлен")
        ...
        try:
            ...
            self.dp.update.middleware(
                AccessControlMiddleware(
                    policy=BanBlacklistPolicy(self.user_storage),
                )
            )
