...
class AdminResponsesMixin:
    ...
    async def create_admin_response_delivered(
        self,
        user_id: int,
        message_text: str,
        admin_id: Optional[int] = None,
    ) -> Optional[int]:
        """Запись ответа отдела продаж уже доставленного в личку (без polling)."""
        try:
            async with self.get_connection() as conn:
                response_id = await conn.fetchval(
                    """
                    INSERT INTO admin_responses (user_id, message_text, admin_id, status, sent_at)
                ...
                logger.info(
                    "✅ Admin response logged as sent immediately: id=%s, user_id=%s",
                    response_id,
                    user_id,
                )
                return response_id
        except Exception as e:
            logger.error(f"❌ Failed to log delivered admin response: {e}")
            return None
