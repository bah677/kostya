...
from storage.log_util import is_transient_db_error, log_storage_failure
...
class MailingStorage:
    ...
    async def update_campaign_scheduled_at(
        ...
        """Сдвигает ``scheduled_at`` (approve черновика → now)."""
        try:
            async with self.db.get_connection() as conn:
                await conn.execute(
                    """
    ...
    async def get_ready_campaigns(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            log_storage_failure(
                logger,
                "❌ Failed to get ready campaigns",
                e,
                exc_info=not is_transient_db_error(e),
            )
            return []
    ...
    async def recover_stale_mailing_state(
        ...
        *,
        ...
        try:
            ...
            log_storage_failure(
                logger,
                "❌ Failed to recover stale mailing state",
                e,
                exc_info=not is_transient_db_error(e),
            )
