...
ADMIN_CONSOLE_HELP_HTML = (
    ...
    "Полный список команд по уровню доступа: <code>/help</code>\n\n"
    "{report_hint}"
    "/clear_my_chat — удалить <b>свою</b> переписку с ботом в личке (с подтверждением)\n"
    "/churn или /otval — детальный отчёт по отвалу + заключение DeepSeek в личку\n"
    "/graf — пошагово: метрика → период → PNG в личку\n"
    "/gift USER_ID [дней] — лицензия в подарок (клуб «Любящие Бога»)\n"
    "/mailing_funnel [ID …] — воронка кампаний рассылки (внутренние ID)\n"
    "/ref_funnel [KEY …|type:…|search:…] — воронка внешних ref-кампаний\n"
    "/followup_leads — срез активных лидов без лицензии по шагам дожима\n"
    "/admins — список админов (только супер-админ)\n"
    "/admin_add USER_ID [note] — добавить админа (цифровой Telegram ID)\n"
    "/admin_del USER_ID — удалить админа (цифровой Telegram ID)\n"
    "\n"
    "<b>Рассылки</b>\n"
    "• <code>/new_mailing</code> — мастер создания кампании (медиа — загрузками, см. шаги).\n\n"
    "<b>Топики</b>\n"
    "• Ответьте <b>reply</b> на пост поддержки с номером тикета — ответ сразу пользователю, тикет закрывается.\n"
    "• Ответьте в топике диалога на карточку с User ID — сообщение сразу в личку (и запись в журнале).\n\n"
    "<i>Не перенесено с legacy:</i> /donation.\n"
)


def admin_console_help_html(*, report_hint: str) -> str:
    return ADMIN_CONSOLE_HELP_HTML.format(report_hint=report_hint)
