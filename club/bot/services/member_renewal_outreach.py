"""Персонализированные напоминания о продлении (AI + fallback на шаблон)."""
...
from bot.texts.prompts.member_renewal_compose import RENEWAL_COMPOSE_SYSTEM
...
async def generate_renewal_outreach_html(
    *,
    user_storage,
    ...
    if rag_stack is not None:
        try:
            ...
            logger.warning("renewal RAG uid=%s: %s", user_id, e)
    ...
    async def _generate(extra: str = "") -> Optional[str]:
        ...
        await _log_llm(user_storage, user_id, "renewal_compose", resp.usage)
    ...
    draft = await run_with_verifier_retries(
        ...
        verify_kwargs={
            "client": llm_client,
            "user_message": f"напоминание о продлении за {days_before} дней",
