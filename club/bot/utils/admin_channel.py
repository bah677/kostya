"""
Отправка сообщений в админский чат основным ботом (TELEGRAM_BOT_TOKEN).
"""
...
def resolve_admin_service_thread_id() -> Optional[int]:
    """Топик для служебных уведомлений в форуме админов.

    В супергруппе с топиками отправка без ``message_thread_id`` идёт в «General»;
    если он закрыт — ``TOPIC_CLOSED``, хотя топик оплат (``PAYMENT_THREAD_ID``) открыт.
    """
    for tid in (config.CLUB_REPORT_THREAD_ID, config.PAYMENT_THREAD_ID):
        if tid and tid > 0:
            return int(tid)
    return None
...
async def send_admin_html_message_main_bot(
    ...
    *,
    ...
    """Отправка в админ-канал основным ботом.

    Возвращает ``message_id`` последнего отправленного фрагмента или ``None``.
    """
...
async def edit_admin_channel_message(
    ...
    *,
    ...
    """Редактирует сообщение в админ-канале основным ботом (как ответ по тикету ТП)."""
...
async def send_admin_document_bytes(
    bot: Bot,
    *,
    data: bytes,
    filename: str,
...
async def send_admin_video_bytes(
    bot: Bot,
    *,
    data: bytes,
    filename: str,
    ...
    if cid is None:
        return False
    vid = BufferedInputFile(data, filename=filename)
    kwargs: dict[str, Any] = {
        "chat_id": cid,
        "video": vid,
        "caption": caption,
...
async def send_admin_voice_bytes(
    bot: Bot,
    *,
    data: bytes,
    filename: str,
...
async def send_admin_animation_bytes(
    bot: Bot,
    *,
    data: bytes,
    filename: str,
    ...
    if cid is None:
        return False
    anim = BufferedInputFile(data, filename=filename)
    kwargs: dict[str, Any] = {
        "chat_id": cid,
        "animation": anim,
        "caption": caption,
...
async def send_admin_video_note_bytes(
    bot: Bot,
    *,
    data: bytes,
    filename: str,
    ...
    if cid is None:
        return False
    vn = BufferedInputFile(data, filename=filename)
    kwargs: dict[str, Any] = {"chat_id": cid, "video_note": vn}
    return await _send_admin_media_with_topic_fallback(
