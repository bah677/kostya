...
_HELP_TEXT = (
    ...
    "<code>/admin_add</code> &lt;id&gt; · <code>/admin_block</code> &lt;id&gt; · "
    "<code>/rag_clear</code> · <code>/rag_topics</code>\n"
    "<code>/telemost_status</code> — почта Телемоста → RAG (статус)\n"
    "<code>/telemost_load №</code> — повторить решение по встрече (если случайно «Игнорировать»)\n"
    "<code>/telemost_poll</code> — опросить почту вручную\n"
    "<code>/ydisk_status</code> — Яндекс.Диск → RAG (статус)\n"
    ...
    "<code>/audio_cut</code> — аудио ~1 мин → голосовые "
    "(топик шортсов в админ-группе)"
)
...
class AppCommandHandlers:
    ...
    def register_handlers(self) -> None:
        ...
        logger.info(
            ...
            "/telemost_status /telemost_poll /telemost_load /ydisk_status /ydisk_sync; /rag_backfill в админ-ветке; "
